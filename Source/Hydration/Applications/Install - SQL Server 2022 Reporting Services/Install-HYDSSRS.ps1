...
# Validation
if (!(Test-Path -path $SetupFile)) {Write-HYDLog "Could not find SQL Server 2022 Reporting Services Setup files, aborting..." -LogLevel 2;Break}
...
function Get-ConfigSet(){
	return Get-WmiObject -Namespace "root\Microsoft\SqlServer\ReportServer\RS_SSRS\v16\Admin" -class MSReportServer_ConfigurationSetting -ComputerName localhost
}
...
If (! $configset.IsInitialized)
{
    ...
    Import-Module "C:\Program Files (x86)\Microsoft SQL Server\160\Tools\PowerShell\Modules\sqlps\sqlps.psd1" -DisableNameChecking | Out-Null
	...
	$inst = Get-WmiObject -Namespace "root\Microsoft\SqlServer\ReportServer\RS_SSRS\v16" -class MSReportServer_Instance -ComputerName localhost
